...
- Request metrics now populate the existing `route` label from the matched
  route pattern. `http_server_requests_total` and
  `http_server_request_duration_seconds` also include `controller` and
  `action` labels for matched controller routes, while non-controller or
  unmatched requests use `unknown`.
...
- `golazy.dev/lazymcp` can register GoLazy MCP modules, serve MCP Streamable
  ...
  UI-enabled tools advertise their app resource with the MCP Apps
