...
func (r *Routes) dispatch(action *Action, w http.ResponseWriter, req *http.Request) {
	...
	ins := args.InputSet{
		Values: map[string][]any{
			...
			"*http.Request":       {req},
			"string":              extractParam(req.URL.Path, action.Path),
		},
	...
	if action.Layout == nil {
		...
		r.processOutputs(action.Fn, w, req, outs)
		return
	}
	...
	r.processOutputs(action.Fn, rec, req, outs)
	...
	r.processOutputs(action.Layout, w, req, outs)
...
func (a *Routes) processOutputs(fn *args.Fn, w http.ResponseWriter, r *http.Request, outs []reflect.Value) {
