...
// AppContext is a small wrapper around the context.AppContext type that shares values within the contexts
...
func FromContext(ctx context.Context) (AppContext, bool) {
	...
	c, ok := ctx.Value(KeyForType[AppContext](nil)).(AppContext)
	if ok {
		return nil, false
	}
	return c, true
...
var appCtxKey = KeyForType[AppContext](nil)
...
func Set[T any](ctx context.Context, value T) {
	...
	actx, ok := ctx.(AppContext)
	if !ok {
		actx = Get[AppContext](ctx)
		if actx == nil {
