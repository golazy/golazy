...
func TestAppContextIsPresent(t *testing.T) {
	...
	lctx2 := Get[LazyContext](New())
	if lctx2 == nil {
		t.Error("context is not present in New()")
	}

	lctx2 = Get[LazyContext](NewWithContext(context.Background()))
	if lctx2 == nil {
...
func TestGetSetAppContext(t *testing.T) {
	ctx := New()
	lctx := Get[LazyContext](ctx)
	if lctx == nil {
		t.Error("expected *lazycontext.AppContext, got nil")
	}
	_, ok := lctx.Value(KeyForType[LazyContext](nil)).(LazyContext)
	if !ok {
