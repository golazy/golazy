...
func TestRegisterLazyDevHandlersOpensEditor(t *testing.T) {
	file := filepath.Join(t.TempDir(), "app", "controllers", "home.go")
	writeLazyDevHandlerFile(t, file, "package controllers\n")
	...
	controlPlane.ServeHTTP(response, httptest.NewRequest(http.MethodPost, LazyDevOpenEditorPath, bytes.NewReader(requestBody)))
...
func writeLazyDevHandlerFile(t *testing.T, filename string, content string) {
	t.Helper()
