...
func NewDecoder() *Decoder {
	...
	registerDefaultConverters(decoder)
	return decoder
}
...
func (d *Decoder) Decode(dst interface{}, src map[string][]string) error {
	...
	if inputErrors := d.validateInput(src); len(inputErrors) > 0 {
		return inputErrors
	}
	for path, values := range src {
		if parts, err := d.cache.parsePath(path, t); err == nil {
			if err = d.decode(v, path, parts, values); err != nil {
				decodeErrors[path] = err
			}
		} else if !errors.Is(err, errInvalidPath) {
			decodeErrors[path] = err
		} else if !d.ignoreUnknownKeys {
			decodeErrors[path] = UnknownKeyError{Key: path}
		}
	}
	decodeErrors.merge(d.setDefaults(t, v))
	decodeErrors.merge(d.checkRequired(t, src))
...
func (d *Decoder) validateInput(src map[string][]string) MultiError {
	errors := MultiError{}
	if d.maxKeys >= 0 && len(src) > d.maxKeys {
		errors["*"] = fmt.Errorf("lazyschema: form has %d keys, larger than max %d", len(src), d.maxKeys)
		return errors
	}
	for key, values := range src {
		if d.maxKeyLength >= 0 && len(key) > d.maxKeyLength {
			errors[key] = fmt.Errorf("lazyschema: key is %d bytes, larger than max %d", len(key), d.maxKeyLength)
			continue
		}
		if d.maxValuesPerKey >= 0 && len(values) > d.maxValuesPerKey {
			errors[key] = fmt.Errorf("lazyschema: key has %d values, larger than max %d", len(values), d.maxValuesPerKey)
			continue
		}
		if d.maxDepth >= 0 && pathDepth(key) > d.maxDepth {
			errors[key] = fmt.Errorf("lazyschema: key depth is larger than max %d", d.maxDepth)
		}
	}
	return errors
}
...
func (d *Decoder) findRequiredFields(t reflect.Type, canonicalPrefix, searchPrefix string) (map[string][]fieldWithPrefix, MultiError) {
	...
	for _, f := range struc.fields {
		if f.typ.Kind() == reflect.Struct {
			fcprefix := joinKey(canonicalPrefix, f.canonicalAlias)
			for _, fspath := range f.paths(prefixKey(searchPrefix)) {
		...
		if f.isRequired {
			key := joinKey(canonicalPrefix, f.canonicalAlias)
			m[key] = append(m[key], fieldWithPrefix{
				fieldInfo: f,
				prefix:    prefixKey(searchPrefix),
			})
		}
	}
	return m, errs
...
func joinKey(prefix string, part string) string {
	if prefix == "" {
		return part
	}
	...
	if conv == nil && t.Kind() == reflect.Slice && m.IsSliceElement {
		...
		if conv == nil {
			...
			if conv == nil {
				...
				return fmt.Errorf("lazyschema: converter not found for %v", elemT)
			}
		}
		...
		if conv != nil {
			...
			return fmt.Errorf("lazyschema: converter not found for %v", t)
		}
	}
	return nil
}
...
func (e ConversionError) Error() string {
	...
	if e.Index < 0 {
		output = fmt.Sprintf("lazyschema: error converting value for %q", e.Key)
	} else {
		output = fmt.Sprintf("lazyschema: error converting value for index %d of %q",
			e.Index, e.Key)
	}
...
func (e UnknownKeyError) Error() string {
	return fmt.Sprintf("lazyschema: invalid path %q", e.Key)
}
