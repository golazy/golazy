...
func NewEncoder() *Encoder {
	encoder := &Encoder{cache: newCache(), regenc: make(map[reflect.Type]encoderFunc)}
	registerDefaultEncoders(encoder)
	return encoder
}
...
func (e *Encoder) encode(v reflect.Value, dst map[string][]string) error {
	...
	if v.Kind() != reflect.Struct {
		return errors.New("lazyschema: interface must be a struct")
	}
	t := v.Type()
	...
	for i := 0; i < v.NumField(); i++ {
		...
		if encFunc == nil {
			errors[v.Field(i).Type().String()] = fmt.Errorf("lazyschema: encoder not found for %v", v.Field(i))
			continue
		}
